...
def command_method(method):
    def wrapper(self, req, *args, **kwargs):
        ...
        mv = body.get('mobileVlan')
        vgw = body.get('vgw')
        DB_PATH = './ryu/app/mobile_vlan/database/CONTROLLER_DATA.db'
        #login in, send flow to host-direct-connect-ovs
        if None != mv:
            #TODO:update database
            print 'Accept push_vlan data from Gateway:'
            mac_addr, vlan_id, user_id = mv['MAC_ADDR'], mv['VLAN_ID'], mv['USER_ID']
            body.pop('mobileVlan')
            print 'MAC_ADDR:{mac},VLAN_ID:{vlan}'.format(mac=mac_addr,vlan=vlan_id)
            
            db = database(DB_PATH)
            if 0 == len(db.findDEVICEByX(x='MAC_ADDR',value=mac_addr)):
                print 'New device! insert to database'
                db.insertDEVICE(mac_addr,vlan_id, user_id)
                printDataSet(db.selectDEVICE()) 
            else:
                print 'Device exsisted! Update database.'
                db.updateDEVICE(mac_addr,vlan_id, user_id)
                printDataSet(db.selectDEVICE())
            #TODO:find dpid
            dpid = db.getDPIDBySlave(mac_addr=mac_addr)
            body['dpid'] = dpid
            if None == dpid:
                LOG.exception('No dpid has this slave!')
                return Response(status=400)
            #TODO:update inport
            in_port = db.findDPIDByX(dpid=dpid, x='MAC_ADDR',value=mac_addr)[0][0]
            body['match']['in_port'] = in_port
        #login in, send flow to gw-direct-connect-ovs
        elif None != vgw:
            db = database(DB_PATH)
            mac_addr = vgw
            body.pop('vgw')
